#[macro_export]
macro_rules ! phantom_tuple_n_reflect {
    ($ident:ident) => {
        impl<const N: usize, T> TupleStruct for $ident<N, T>
        where
        ...
        unsafe impl<const N: usize, T> Reflect for $ident<N, T>
        where
            T: 'static + Send + Sync + Serialize,
        {
        ...
        impl<const N: usize, T> GetTypeRegistration for $ident<N, T>
        where
            T: 'static + Send + Sync + Serialize,
        {
