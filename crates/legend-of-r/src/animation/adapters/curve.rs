    ...
    fn animate(world: &mut World, animation: Entity, time: AnimationTime) -> Self::Type {
        info_span!("Curve").in_scope(|| {
            ...
            Self::handle::<T>(world, animation).animate(
                world,
                AnimationTime {
                    t: time.t.powf(exponent),
                    prev_t: time.prev_t.powf(exponent),
                    ..time
                },
            )
        })
...
impl<T> TimelineAnimation for Curve<T>
where
    T: 'static + Send + Sync + Default,
{
    fn visit(
        ...
        for animation in animation_ui.plot_widgets.iter_mut() {
            match animation {
                AnimationWidget::TimelineAnimation(widget) => {
                    widget.min_t = widget.min_t.map(|min_t| min_t.powf(1.0 / data.exponent));

                    widget.max_t = widget.max_t.map(|max_t| max_t.powf(1.0 / data.exponent));

                    if let Some(max_t) = widget.max_t {
                        timeline_ui.length = timeline_ui.length.max(max_t)
                    }
                }
                AnimationWidget::Stop(widget) => widget.x = widget.x.powf(1.0 / data.exponent),
...
pub trait CurveTrait<'w, 's, 'a, I, T>
where
    T: Animate,
    T::Type: 'static + Send + Sync + Default,
{
    fn with_curve(self, offset: f64) -> AnimationEntityBuilder<'a, I, Curve<T::Type>>;
}

impl<'w, 's, 'a, I, T> CurveTrait<'w, 's, 'a, I, T> for AnimationEntityBuilder<'a, I, T>
where
    I: AnimationBuilderTrait,
    T: TimelineAnimation,
    T::Type: Default,
{
    fn with_curve(self, exponent: f64) -> AnimationEntityBuilder<'a, I, Curve<T::Type>> {
        let child = self.id();
